...
class DbBackupConfigure(models.Model):
    ...
    log_ids = fields.One2many('db.backup.log', 'config_id',
                              string='Backup Logs')
    ...
    def _compute_log_count(self):
        """Count the number of backup log entries for the smart button"""
        for rec in self:
            rec.log_count = len(rec.log_ids)

    @staticmethod
    def _format_backup_error(error):
        """Format an exception for the backup log. Includes the exception
        type so the log message is never blank, since some low-level
        connection/timeout errors have an empty str()."""
        text = str(error)
        return f"{type(error).__name__}: {text}" if text else type(
            error).__name__

    def _gdrive_resumable_upload(self, rec, file_path, backup_filename):
        """Upload a backup file to Google Drive using the resumable upload
        protocol. Google recommends this over a single multipart POST for
        anything beyond a few MB - a plain multipart request has no way to
        resume or clearly fail on a dropped connection, which is what a
        multi-GB database backup runs into. Explicit timeouts make sure a
        dead connection raises instead of hanging indefinitely."""
        headers = {
        ...
        if not upload_url:
            raise UserError(
                _("Google Drive did not return an upload session URL."))
        file_size = os.path.getsize(file_path)
        with open(file_path, 'rb') as upload_file:
            # (connect timeout, read timeout) - a long read timeout is
            # needed since a multi-GB upload can legitimately take a while,
            # but it still guarantees the request eventually fails instead
            # of hanging forever.
        ...
        if upload_response.status_code not in (200, 201):
            # Covers both error responses (4xx/5xx) and a 308 "Resume
            # Incomplete" - raise_for_status() alone would miss the 308
            # case since it only raises for 4xx/5xx, leaving an unclear
            # JSON-parse error instead of a clear upload failure.
            raise UserError(_(
                "Google Drive upload failed (HTTP %s): %s",
                upload_response.status_code, upload_response.text[:500]))
        return upload_response.json()

    def action_view_backup_logs(self):
        """Open the backup logs related to this configuration"""
        self.ensure_one()
        return {
    ...
    def _schedule_auto_backup(self, frequency):
        ...
        for rec in records:
            ...
            log = self.env['db.backup.log'].sudo().create({
                'config_id': rec.id,
                'db_name': rec.db_name,
                'backup_destination': rec.backup_destination,
            ...
            if rec.backup_destination == 'local':
                try:
                    ...
                    log.write({'state': 'failed', 'message': self._format_backup_error(e)})
            ...
            # Google Drive backup
                ...
                try:
                    ...
                    try:
                        headers = {
                            "Authorization": "Bearer %s" % rec.gdrive_access_token}
                        # Resumable upload: reliable for large (multi-GB)
                        # backups, with explicit timeouts and a checked
                        # response instead of a single multipart POST that
                        # could hang or fail silently.
                        self._gdrive_resumable_upload(rec, temp.name,
                                                      backup_filename)
                        if rec.auto_remove:
            ...
            # Dropbox backup
                ...
                try:
                    ...
                    log.write({'state': 'failed', 'message': self._format_backup_error(error)})
            ...
            # Onedrive Backup
                ...
                try:
                    ...
                    log.write({'state': 'failed', 'message': self._format_backup_error(req_error)})
                ...
                if rec.aws_access_key and rec.aws_secret_access_key:
                    try:
                        ...
                        if rec.aws_folder_name in prefixes:
                            ...
                            # Destination folder doesn't exist in the bucket,
                            # so nothing was uploaded.
                            log.write({
                                'state': 'failed',
                                'message': _(
                                    "Folder '%s' not found in bucket '%s'.",
                                    rec.aws_folder_name, rec.bucket_file_name)
                            })
                    except Exception as error:
